    ...
    it('should return latest valuation from database if site doesn\'t give a proper valuation', function (done) {
      ...
      valuationUtils.getValuation(fund)
        .then((valuation) => {
          ...
          assert.equal(
            valuation.date.getDay(),
            yesterday.getDay(),
            'expected valuation to be the latest from the database'
          );
          assert.equal(
            valuation.value,
            106.32,
          ...
          assert.isNotNull(valuation.change);
          assert.isNotNull(valuation.previousValue);
          assert.isNotNull(valuation.date);
        })
        .then(() => databaseMock.verify())
        .then(() => valuationApiMock.verify())
        .then(() => done())
