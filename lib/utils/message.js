...
function getFundMissingReaction() {
  return reactions.noFundInfoFromFundSquare[numberUtils.getRandomNumber(reactions.noFundInfoFromFundSquare)];
}
...
function sumInvested(invested) {
  if (invested.value) {
    return invested.reduce((prev, next) => prev + next.value, 0);
  }
  return 0;
}
...
function formatValuation(valuation, sharesCount, fundType) {
  ...
  const summary = sharesCount !== 1 ? `The value of your ${sharesCount} shares is now ${formatCurrency2(valuation.value * sharesCount)} DKK (${prefix}${change} DKK).` : '';
...
function formatInvestment(invested, fundType) {
  ...
  if (!invested.value) {
    return `${formatFundType(fundType)} is not available at the moment.`;
  } else if (!isLatestValuationValid) {
...
function getTotalInvestment(invested, fundType, action) {
  ...
  if (!invested.date) {
    return getFundMissingReaction();
  }
  // const intro = 'The total invested value';
  const sumInvestedValue = sumInvested(invested);
  const formattedTotal = formatCurrency(sumInvestedValue);
