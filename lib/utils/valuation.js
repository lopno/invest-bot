...
function getValuation(type) {
  var valueAlreadyInDatabase; // eslint-disable-line no-var
  var databaseValuation = null; // eslint-disable-line no-var
  return database.getLatestValuations(2)
    .then((valuations) => {
      ...
      if (valuations && valuations.length === 2) {
        databaseValuation = {
          ...
          change: numberUtils.getPercentageChange(valuations[1][type], valuations[0][type]),
    ...
    .then((valuation) => {
      ...
      throw new Error('Could not find any valuation');
    })
    .then(valuation =>
      Promise.all([
        valuation,
